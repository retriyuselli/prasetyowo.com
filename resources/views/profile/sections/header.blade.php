        ...
        <div class="relative group">
            @if($user->avatar_url)
                <img class="h-24 w-24 rounded-full object-cover border-4 border-white shadow-lg transition-transform duration-300 group-hover:scale-105"
                    ...
                    onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&color=ffffff&background=1e40af&size=128&font-size=0.4'">
            @else
                <img class="h-24 w-24 rounded-full object-cover border-4 border-white shadow-lg transition-transform duration-300 group-hover:scale-105"
                    src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&color=ffffff&background=1e40af&size=128&font-size=0.4"
                    alt="Profile {{ $user->name }}">
            @endif
